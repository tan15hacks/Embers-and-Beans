...
import { CheckCircle2, CreditCard, Landmark, Minus, Plus, Smartphone, ShoppingBag, X } from "lucide-react";
...
const paymentOptions = [
  {
    ...
    label: "GCash",
    description: "Pay through GCash after the shop confirms availability.",
    icon: Smartphone,
  },
  {
    value: "bank" as const,
    label: "Bank Transfer",
    description: "Pay by bank transfer after confirmation.",
    icon: Landmark,
  },
];
...
export function OrderForm({ items }: OrderFormProps) {
  ...
  function validateOrder() {
    if (selectedItems.length === 0) {
      return "Please choose at least one item before submitting.";
    }
    ...
    if (details.customerEmail.trim() && !/^\S+@\S+\.\S+$/.test(details.customerEmail.trim())) {
  ...
  function openPaymentStep(event: FormEvent<HTMLFormElement>) {
  ...
  async function submitOrder() {
    ...
    try {
      ...
      setStatus("success");
      setStatusMessage(payload.message ?? "Order request received. We will confirm by phone or email.");
  ...
  return (
    <>
      <form onSubmit={openPaymentStep} className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
        ...
        <aside className="rounded-[2rem] border border-[#2B1E18]/10 bg-[#FFFDFB] p-6 shadow-[0_24px_90px_rgba(43,30,24,0.1)] lg:sticky lg:top-28">
          ...
          <div className="mt-6 grid gap-4">
            ...
            <div>
              ...
              <input id="customerPhone" required inputMode="tel" className={inputClass} value={details.customerPhone} onChange={(event) => updateDetails("customerPhone", event.target.value)} placeholder="+63 912 345 6789" />
            </div>
            <div>
              <label htmlFor="customerEmail" className={labelClass}>Email optional</label>
              <input id="customerEmail" type="email" className={inputClass} value={details.customerEmail} onChange={(event) => updateDetails("customerEmail", event.target.value)} placeholder="you@email.com" />
            </div>
            <div>
      ...
      {isPaymentOpen && (
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-[#2B1E18]/60 px-4 py-8 backdrop-blur-sm">
          <div className="w-full max-w-xl rounded-[2rem] bg-[#FFFDFB] p-6 text-[#2B1E18] shadow-[0_30px_120px_rgba(43,30,24,0.28)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="flex items-center gap-2 text-xs font-bold uppercase tracking-[0.22em] text-[#B7793C]">
                  <CreditCard size={16} /> Payment Method
                </p>
                <h2 className="mt-3 font-[var(--font-display)] text-4xl font-semibold">
                  Choose how you want to pay.
                </h2>
                <p className="mt-3 text-sm leading-6 text-[#4A342A]/70">
                  No online charge yet. The shop will send the final payment instructions after confirming your order.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setIsPaymentOpen(false)}
                className="flex size-10 shrink-0 items-center justify-center rounded-full border border-[#2B1E18]/10 text-[#4A342A]/70 transition hover:bg-[#F8F4EF]"
                ...
                <label
                  ...
                  className={`flex cursor-pointer items-start gap-4 rounded-2xl border p-4 transition ${
                    paymentMethod === option.value
                      ? "border-[#B7793C] bg-[#B7793C]/10"
                      : "border-[#2B1E18]/10 bg-[#F8F4EF] hover:bg-[#F8F4EF]/70"
                  }`}
                >
                  <input
                    type="radio"
                    ...
                    className="mt-2"
                  />
                  <div className="flex size-11 shrink-0 items-center justify-center rounded-full bg-[#FFFDFB] text-[#B7793C]">
                    <option.icon size={20} />
                  </div>
                  <div>
                    <p className="font-semibold text-[#2B1E18]">{option.label}</p>
                    <p className="mt-1 text-sm leading-6 text-[#4A342A]/70">{option.description}</p>
                  </div>
                </label>
              ))}
            </div>
