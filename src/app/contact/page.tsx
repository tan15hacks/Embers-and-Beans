...
const orderSteps = [
  "Send your preferred drinks, pastries, and pickup time.",
  "We confirm availability and total price by email.",
  "Your order is prepared close to pickup so it stays fresh.",
];
          ...
          <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
            {contactItems.map((item, index) => (
              <Reveal key={item.title} delay={index * 0.04}>
                <article className="h-full rounded-[2rem] border border-[#2B1E18]/10 bg-[#FFFDFB] p-8 shadow-[0_20px_80px_rgba(43,30,24,0.06)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_90px_rgba(43,30,24,0.1)]">
      ...
      <section className="pb-24">
        <Container>
          <div className="grid gap-8 lg:grid-cols-[0.82fr_1.18fr] lg:items-start">
            <Reveal>
              <div className="overflow-hidden rounded-[2.5rem] border border-[#2B1E18]/10 bg-[#FFFDFB] shadow-[0_30px_100px_rgba(43,30,24,0.12)] lg:sticky lg:top-28">
                <div className="relative min-h-[620px] p-8 text-[#FFFDFB]">
                  <Image
                    ...
                    className="object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#2B1E18]/92 via-[#2B1E18]/35 to-transparent" />
            ...
            <Reveal delay={0.08}>
              <div className="rounded-[2.5rem] border border-[#2B1E18]/10 bg-[#FFFDFB] p-6 shadow-[0_30px_100px_rgba(43,30,24,0.1)] sm:p-10">
                ...
                <div className="mt-8 rounded-[2rem] bg-[#F8F4EF] p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#B7793C]">
                    How order-ahead works
                  </p>
                  <div className="mt-5 grid gap-3">
                    {orderSteps.map((step, index) => (
                      <div key={step} className="flex gap-3 rounded-2xl bg-[#FFFDFB] p-4">
                        <span className="flex size-8 shrink-0 items-center justify-center rounded-full bg-[#2B1E18] text-sm font-bold text-[#FFFDFB]">
                          {index + 1}
                        </span>
                        <p className="text-sm leading-6 text-[#4A342A]/75">{step}</p>
                      </div>
                    ))}
                  </div>
                </div>
